...
class Bucket {
	...
	update(key, value) {
		...
		// loop through arrays in bucket, if key is in one of the arrays, update the value
		for (let i = 0; i < this.bucket.length; i++) {
			let keyValue = this.bucket[i];									// grab 1D array [ key, value ]

			if (keyValue[0] === key) {											// key found!
				keyValue[1] = value;													// update value
				found = true;
				return;																				// exit
			} 
		}
...
var MyHashMap = function() {
	this.keySpace = 2069;									// use prime number to minimmize # of collisions (can happen if keys have patterns ex. multiples of 2's)
	this._hashTable = [];									// use underlying array, each value is a bucket object (array)

	// each value in array is a new bucket object (array-like for simplicity instead of linked list)
...
MyHashMap.prototype._hashify = function(key) {	// HASHING NUMBERS ONLY
	return key % this.keySpace;                  	// 2069 == bin size (use prime number)
}
...
MyHashMap.prototype.put = function(key, value) {
	let hash = this._hashify(key);								// convert key to hashed value
	let bucket = this._hashTable[hash];						// grab bucket located at hash address
...
MyHashMap.prototype.get = function(key) {
	let hash = this._hashify(key);								// convert key to hashed value
	let bucket = this._hashTable[hash];						// grab bucket located at hash address
	return bucket.get(key);												// get value in buckets 
};
...
MyHashMap.prototype.remove = function(key) {
	...
	let bucket = this._hashTable[hash];
	bucket.remove(key);
};
...
console.log(obj._hashTable[0]);
// console.log(' ');
// obj.put(2069, 7);			// should have hash collison (w/ 0 key)!
// console.log(obj._hashTable.slice(0,5));